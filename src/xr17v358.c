...
static uintptr_t xr17v358_register_address(volatile void *device_base,
                                           size_t port_index,
                                           uint32_t register_offset) {
  return (uintptr_t)device_base + k_port_offsets[port_index] + register_offset;
}

static size_t xr17v358_read_fifo_level(volatile void *device_base,
                                       size_t port_index,
                                       uint32_t register_offset) {
  const volatile uint8_t *reg = (volatile uint8_t *)xr17v358_register_address(
      device_base, port_index, register_offset);
  size_t level = *reg;

  if (level > XR17V358_FIFO_CAPACITY) {
    level = XR17V358_FIFO_CAPACITY;
  }
...
static xr17v358_error xr17v358_stage_rx_bytes_from_hw(volatile void *device_base,
  ...
  bytes_available =
      xr17v358_read_fifo_level(device_base, port_index, XR17V358_REG_RXLVL);
  if (bytes_available == 0U) {
    return XR17V358_OK;
  }

  bytes_to_stage = rx_fifo[port_index].capacity - rx_fifo[port_index].size;
  if (bytes_to_stage > bytes_available) {
    bytes_to_stage = bytes_available;
  }
  if (bytes_to_stage > sizeof(frame_bytes)) {
    bytes_to_stage = sizeof(frame_bytes);
  }
  ...
  error = xr17v358_read_rx_fifo(device_base, port_index, frame_bytes,
                                bytes_to_stage, &bytes_read);
  if (error != XR17V358_OK) {
    return error;
  }

  if (xr17v358_ring_write_bytes(&rx_fifo[port_index], frame_bytes,
...
static xr17v358_error xr17v358_flush_tx_queue_to_hw(volatile void *device_base,
                                                    size_t port_index) {
  uint8_t frame_bytes[XR17V358_FIFO_CAPACITY];
  size_t bytes_available = 0U;
  size_t bytes_to_write = 0U;
  size_t bytes_written = 0U;
  xr17v358_error error;

  bytes_available =
      xr17v358_read_fifo_level(device_base, port_index, XR17V358_REG_TXLVL);
  if (bytes_available == 0U || tx_queue[port_index].size == 0U) {
    return XR17V358_OK;
  }

  bytes_to_write = tx_queue[port_index].size;
  if (bytes_to_write > bytes_available) {
    bytes_to_write = bytes_available;
  }
  if (bytes_to_write > sizeof(frame_bytes)) {
    bytes_to_write = sizeof(frame_bytes);
  }

  bytes_to_write = xr17v358_ring_read_bytes(&tx_queue[port_index], frame_bytes,
                                            bytes_to_write);
  if (bytes_to_write == 0U) {
    return XR17V358_OK;
  }

  error = xr17v358_write_tx_fifo(device_base, port_index, frame_bytes,
...
xr17v358_error xr17v358_get_uart_base(size_t port_index, uint32_t *uart_base) {
  const uint32_t device_base = XR17V358_DEVICE_BASE_ADDRESS;
...
xr17v358_error xr17v358_read(size_t port_index, uint8_t *data, size_t length,
  ...
  return xr17v358_read_from_staged_rx(port_index, data, length, bytes_read);
}
...
xr17v358_error xr17v358_read_rx_fifo(volatile void *device_base,
  ...
  if (device_base == NULL || bytes_read == NULL ||
      (length > 0U && data == NULL)) {
    return XR17V358_ERROR_INVALID_ARGUMENT;
  }
  ...
  for (i = 0U; i + sizeof(uint32_t) <= count; i += sizeof(uint32_t)) {
    const uint32_t word = fifo_words[i / sizeof(uint32_t)];

    data[i] = (uint8_t)(word & 0xFFU);
    data[i + 1U] = (uint8_t)((word >> 8U) & 0xFFU);
    data[i + 2U] = (uint8_t)((word >> 16U) & 0xFFU);
    data[i + 3U] = (uint8_t)((word >> 24U) & 0xFFU);
  }
...
xr17v358_error xr17v358_write_hw(volatile void *device_base, size_t port_index,
  ...
  *bytes_written = 0U;
  ...
  error = xr17v358_flush_tx_queue_to_hw(device_base, port_index);
  if (error != XR17V358_OK) {
    return error;
  }
...
xr17v358_error xr17v358_read_hw(volatile void *device_base, size_t port_index,
  ...
  error = xr17v358_stage_rx_bytes_from_hw(device_base, port_index);
  if (error != XR17V358_OK) {
    return error;
  }
...
xr17v358_error xr17v358_set_discrete(volatile void *device_base,
  ...
  mcr = (volatile uint8_t *)xr17v358_register_address(device_base, port_index,
                                                      XR17V358_REG_MCR);
  value = *mcr;
  if (asserted) {
    value &= (uint8_t)~XR17V358_MCR_RTS;
  } else {
    value |= XR17V358_MCR_RTS;
  }
  *mcr = value;
  return XR17V358_OK;
}
...
xr17v358_error xr17v358_set_loopback(volatile void *device_base,
  ...
  value = *mcr;
  if (enabled) {
    value |= XR17V358_MCR_LOOPBACK;
  } else {
    value &= (uint8_t)~XR17V358_MCR_LOOPBACK;
  }
  *mcr = value;
  return XR17V358_OK;
}
